...
export default function Player() {
  ...
  // 👇 Add shading type after ball color
  const { "ball color": ballColor, "ball texture": ballShading } = useControls({
    "ball color": "#be3737",
    "ball texture": {
      value: "flat",
      options: ["flat", "phong", "gouraud"],
    },
  });
  ...
  // Helper to select correct material based on shading
  const getBallMaterial = () => {
    const color = isBurning ? burnedColor : ballColor;

    switch (ballShading) {
      case "phong":
        return <meshPhongMaterial color={color} shininess={60} />;
      case "gouraud":
        return <meshLambertMaterial color={color} />;
      default:
        return <meshStandardMaterial flatShading color={color} />;
    }
  };
  ...
  useFrame((state, delta) => {
    ...
    if (
      ...
      if (useGame.getState().currentLevel === lastLevel) finish();
      else end();
    }
